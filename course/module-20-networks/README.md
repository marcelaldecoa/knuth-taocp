...
`n` elements makes at least `⌈lg n!⌉` comparisons in the worst case.
...
`2^h ≥ (number of leaves) ≥ n!`, so `h ≥ lg n!`. Because `h` is an integer,
`h ≥ ⌈lg n!⌉`. ∎
...
Using `lg n! = Σ_{k≤n} lg k ≈ n lg n − n/ln 2` (Stirling), the bound is
...
|---|---|---|---|---|---|---|---|---|---|----|----|----|
| ⌈lg n!⌉ | 0 | 1 | 3 | 5 | 7 | 10 | 13 | 16 | 19 | 22 | 26 | 29 |

### The gap between the bound and the truth

Let `S(n)` be the *true* minimum — the fewest comparisons that suffice in the
worst case, taken over the best possible algorithm. Clearly `S(n) ≥ ⌈lg n!⌉`.
Is it equal?

For `n ≤ 11`, **yes**: `S(n) = ⌈lg n!⌉`. But at `n = 12` the counting bound
says 29 while the truth is `S(12) = 30`. There simply is no 29-comparison sort
of 12 elements, even though a binary tree of height 29 has room for `2^29 ≈ 5.4
× 10^8 ≫ 12! = 4.8 × 10^8` leaves. The leaves cannot be *packed* to make every
...
(`S(n) = ⌈lg n!⌉` also for `n ≤ 11` and `n = 20, 21`; it differs at `12–19`
...
**Stage 1** has you compute `⌈lg n!⌉`, a counting sort that *reports* its
...
If `⌈lg n!⌉` is a target, which algorithm *hits* it for small `n`? Not straight
...
search must probe — and binary search into a range of size `m` costs `⌈lg(m+1)⌉`
comparisons.
...
- **M3. Insert.** To insert we have `b_2 = 2` and the leftover `4`.
  ...
  - Insert the leftover `4` into the whole chain `[1,3,5]` by binary search:
    ...
    (2 comparisons; range of size 3, budget `⌈lg 4⌉ = 2`.)
...
Total: `3 + 2 + 2 = 7 = ⌈lg 5!⌉`. Merge insertion is **optimal** for `n = 5`.
...
Compare with `⌈lg n!⌉` above: they agree up to `n = 11`, and at `n = 12`
merge insertion uses 30 — which is exactly `S(12)`. So `F(n) = S(n) = ⌈lg n!⌉`
for `n ≤ 11`, and `F(12) = S(12) = 30 > 29`. Merge insertion is the
comparison-optimal sort for every `n ≤ 15` except a couple of larger cases;
...
**Comparator count.** `C(n) = C(n/2)·2 + M(n)` where the merge uses
`M(n) = M(n/2)·2 + (n/2 − 1)` comparators. Solving: `C(n) = (t²−t+4)·2^{t−1} − 1`
with `t = lg n`, i.e. `Θ(n (lg n)²)`. Concretely:
...
```text
    depth(2^t) = t(t+1)/2 = 1 + 2 + ... + t = O((lg n)²).
```
...
> **Parallel time:** a network sorts in `O((lg n)²)` depth — with `n/2`
...
> SIMD lane never mispredicts. You pay in *total work* (`n (lg n)²` vs `n lg n`)
...
`n = 16` (over `2 × 10^13`). The zero-one principle collapses the work to `2^n`.
...
input but fails on some input `x = (x_1, …, x_n)`, producing an output that is
...
`f(x) = (f(x_1), …, f(x_n))` produces `f(y)` where `y` is the output on `x`.
...
```text
    f(v) = 0 if v < y_k,   1 if v ≥ y_k.
```

`f` is nondecreasing, so the network on `f(x)` outputs `f(y)`. But
`f(y_k) = 1` (since `y_k ≥ y_k`) and `f(y_{k+1}) = 0` (since `y_{k+1} < y_k`), so
...
more comparators than odd-even merge (`n/2 · t(t+1)/2`, e.g. 24 for `n=8`) but
has the same `O((lg n)²)` depth and an extremely regular, index-arithmetic
...
Compute `⌈lg n!⌉` with a wide-integer factorial and a "smallest `c` with
`2^c ≥ n!`" loop. `sort_and_count` can be any correct sort that counts key
comparisons — binary insertion sort is a clean choice. The tests pin the table
of `⌈lg n!⌉` for `n ≤ 12` and dramatize the `S(12) = 30 > 29` gap.
