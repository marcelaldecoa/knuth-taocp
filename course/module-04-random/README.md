...
> the _Loom of Chaos_ plots consecutive pairs $(X_n, X_{n+1})$ and lets you drag a
...
- **Complex $\ne$ random.** Von Neumann's own "middle-square" method (square the
...
$X_0, X_1, X_2, \ldots$ must eventually repeat: it is **ultimately periodic**. A short
...
- **The word-size modulus.** Taking $m = 2^{64}$ makes the "mod" *free*: it is
  ...
  cannot overflow.
- **Indexing.** `new` stores $X_0$; the first `next()` returns $X_1$. Knuth traces
  sequences from $X_1$, and so do the tests.

**Trace (§3.1's opener).** $X_0 = a = c = 7$, $m = 10$:
...
generator visits all of $\mathbb{Z}_m$ before repeating). This happens exactly under
three conditions.
...
$x \mapsto ax + c$ to be a single $m$-cycle. Condition 1 rules out an increment that
...
$X_n = a^n X_0 + c(a^n - 1)/(a - 1) \pmod{m}$, one analyzes, prime power by prime
power via the Chinese Remainder Theorem, when $a^n \equiv 1$ and the geometric-sum
factor first returns to 0. For a prime power $p^e$ the order works out to require
$p \mid (a - 1)$, with the extra $4 \mid (a - 1)$ needed at the prime 2 because the group
of units mod $2^e$ is not cyclic for $e \ge 3$. Knuth gives the full number-theoretic
argument in §3.2.1.2; the payoff is a clean, checkable recipe. ∎

**Worked predicate, $m = 16 = 2^4$.** The only prime dividing $m$ is 2, and $4 \mid m$,
so conditions 2–3 collapse to $a \equiv 1 \pmod{4}$, i.e. $a \in \{1, 5, 9, 13\}$; condition
1 says $c$ must be odd. So full period $\iff$ ($a \bmod 4 = 1$) and ($c$ odd). For example
`period(5, 3, 16, 0) = 16`, while `period(3, 1, 16, 0) < 16` ($3 \equiv 3 \bmod 4$) and
`period(5, 2, 16, 0) < 16` ($c$ even). Stage 1 verifies the *entire* "if and only
if" by enumerating all 256 $(a, c)$ pairs.

**Worked predicate, $m = 100 = 2^2 \cdot 5^2$.** Primes 2 and 5 divide $m$, and $4 \mid m$, so
$a \equiv 1 \pmod{4}$ and $a \equiv 1 \pmod{5}$, i.e. **$a \equiv 1 \pmod{20}$** ($a \in \{1,21,41,61,81\}$);
...
$$X_{n+1} \bmod 2^b = (a \cdot X_n + c) \bmod 2^b$$

depends only on $X_n \bmod 2^b$. So the last $b$ bits are themselves a little LCG
...
which is $\lfloor (X / 2^{64}) \cdot \text{bound} \rfloor$: it reads the *top* bits of X and lands
uniformly in `0..bound`.

### 2.3 The spectral flaw: RANDU and the 9−6−1 identity

IBM's notorious **RANDU** used $a = 65539 = 2^{16} + 3$, $c = 0$, $m = 2^{31}$. Its period
issues aside, it has a fatal *structural* defect. Compute $a^2$ modulo $m$:
...
Since (with $c = 0$) $X_{n+2} = a^2 X_n$ and $X_{n+1} = a X_n$ modulo $m$,
...
unit cube, therefore satisfies one linear equation $9x - 6y + z = (\text{integer}) \cdot 2^{31}$
...
$\sum p_s = 1$). Let $Y_s$ be the observed count in category $s$; the model expects
$n \cdot p_s$. Define the statistic

$$V = \sum_s \frac{(Y_s - n \cdot p_s)^2}{n \cdot p_s}.$$
...
as a **chi-square variate with $\nu = k - 1$ degrees of freedom** (one is lost to
the constraint $\sum Y_s = n$). We reject the model when $V$ exceeds a high percentile
of that distribution. A tiny table of the $\nu = 9$ row (used by stages 2–4):

| percentile | 1% | 25% | 50% | 75% | 95% | 99% |
|---|---|---|---|---|---|---|
| chi-square, $\nu = 9$ | 2.09 | 5.90 | 8.34 | 11.39 | 16.92 | **21.67** |

So with 10 categories a fair generator should give $V$ below $\approx 21.67$ about 99% of
the time. **Rule of thumb (Knuth):** every expected count $n \cdot p_s$ should be $\gtrsim 5$,
...
**Worked example (two dice).** Throw two dice 144 times; category $s = 2, \ldots, 12$
...
$$V = \frac{(2-4)^2}{4} + \frac{(4-8)^2}{8} + \cdots + \frac{(6-4)^2}{4} = \frac{343}{48} \approx 7.15.$$

With $\nu = 10$, that sits between the 25% and 50% points — an unremarkable, and
...
$0 \ldots j$. That is the entire trick.
...
positions $0 \ldots t-1$ and moves that element to the last slot; each of the $t$
choices has probability $1/t$, and each fixes a *distinct* final element. The
remaining $t - 1$ elements occupy positions $0 \ldots t-2$ and are then shuffled by the
...
$(1/t) \cdot (1/(t-1)!) = 1/t!$. Since the outcomes partition all $t!$ permutations, each
occurs with probability $1/t!$. ∎

Equivalently: P consumes one draw from each of the bounds $t, t-1, \ldots, 2$, giving
$t \cdot (t-1) \cdots 2 = t!$ equally likely "tapes", and the argument shows the map
tape $\mapsto$ permutation is a **bijection** onto the $t!$ permutations. Stage 3
enumerates the 6 tapes for $t = 3$ (all distinct outputs) and, for $t = 6$, drives
43 200 shuffles ($\approx 60$ per permutation) through the stage-2 chi-square test over
all 720 permutations ($\nu = 719$, comfortably below the $\approx 810$ that marks the 99%
point).
...
draws each with $n$ outcomes, hence **$n^n$** equally likely tapes. If every one of
the $n!$ permutations were equally likely, $n!$ would have to divide $n^n$. But for
$n > 2$ it does not: $n!$ contains the prime factors of $n-1, n-2, \ldots$, which $n^n$ (a
power of $n$) generally lacks. For $n = 3$: $3^3 = 27$ tapes, $3! = 6$ permutations, and
...
**Theorem (the invariant).** After $t \ge k$ records have been read, every one of
...
$t$ records; consider record $t + 1$. In step R3 we draw $M$ uniform in $0 \ldots t+1$.
...
- An **old** record was present with probability $k/t$ (hypothesis). It survives
  ...
  $1 - 1/(t+1) = t/(t+1)$, giving total $(k/t) \cdot (t/(t+1)) = k/(t+1)$. ✓
...
confirms each item appears with frequency $\approx k/N$ via the chi-square test.
...
$V = \sum (\text{obs} - \text{exp})^2 / \text{exp}$. Panic on length mismatch ("same number"), empty input
...
draw $M$ in $0 \ldots t$ and overwrite slot $M$ when $M < k$.

---

## 6. Check your understanding

1. The generator $X_0 = a = c = 7$, $m = 10$ has period 4. Which condition of
   Theorem 3.2.1.2A fails? ($c = 7$ is coprime to 10 ✓; $a = 7 \equiv 1 \bmod 2$? no —
   fails condition 2 at $p = 2$, and also $a \not\equiv 1 \bmod 5$.)
2. Why is `X % 2` a terrible coin flip for a power-of-two-modulus LCG, but
   `(X · 2) >> 64` fine? (The low bit has period $\le 2$; the high bits carry the
   full period.)
3. For the two-dice example $V \approx 7.15$ with $\nu = 10$ — do you reject the generator?
...
| 3.2.1-2 | 10 | For what seeds does $X_0 = a = c = 7$, $m = 10$ enter its cycle, and how long is any tail? |
...
- **Divisibility obstruction** — the naive shuffle *cannot* be fair because
  $3^3 = 27$ outcomes can't split evenly over 6 permutations; no amount of
