...
that rank them: the $\lfloor \lg N \rfloor + 1$ bound for binary search, the $1.386 \lg n$
average depth of a random binary search tree, the Fibonacci-tree proof that
AVL trees stay $\sim 1.44 \lg n$ tall, and Knuth's 1962 linear-probing formula.
...
> type words and watch an honest $h = (h \cdot 31 + c) \bmod m$ compute each bucket in
...
> $1 - \prod (m-i)/m$ live — with $m = 23$ buckets, six keys already pass 50% — and lets
...
Fix a set of $N$ records with distinct keys $K_1, \ldots, K_N$. A **search**
...
|---|---|---|---|---|
| Unsorted array — sequential | $\Theta(N)$ | $O(1)$ | $N$ | §6.1 |
| Sorted array — **binary** | $\Theta(\lg N)$ | $\Theta(N)$ | $N$ | §6.2.1; insert is dear |
| **Binary search tree** | $\Theta(\lg N)$ avg, $\Theta(N)$ worst | $\Theta(\text{depth})$ | $N$ | §6.2.2 dynamic |
| **AVL (balanced) tree** | $\Theta(\lg N)$ worst | $\Theta(\lg N)$ | $N$ | §6.2.3 guaranteed |
| **Hash table** | $\Theta(1)$ avg | $\Theta(1)$ avg | $\sim N/\alpha$ | §6.4 no order |
...
$\ge \lg N$ levels (§6.2.1, and the same information-theoretic bound as sorting).
...
Search the sorted array $K_1 \le K_2 \le \ldots \le K_N$ for argument $K$.
...
> **Theorem B.** Algorithm B never makes more than $\lfloor \lg N \rfloor + 1$
> comparisons, whether the search succeeds or fails.

*Proof (comparison tree).* Draw the tree of comparisons Algorithm B can make.
The root compares against $K_{\lfloor (1+N)/2 \rfloor}$; its children handle the two
...
$\le \lfloor \lg N \rfloor$ (counting the root as level 0). A successful search stops at an
internal node, costing at most $\lfloor \lg N \rfloor + 1$ comparisons; an unsuccessful
...
For $N = 16$, $\lfloor \lg 16 \rfloor + 1 = 5$; our trace used 4, and no key needs more
...
The catch: keeping the array sorted under insertions costs $\Theta(N)$ per insert
...
>
> $$C_n = 2(1 + 1/n) H_n - 3 \approx 1.386 \lg n,$$
>
...
and a right subtree of size $n - 1 - k$, with $k$ uniform on $\{0, \ldots, n-1\}$.
...
$$S_n = (n - 1) + \frac{1}{n} \sum_{k=0}^{n-1} (S_k + S_{n-1-k}).$$
...
$H_n \approx \ln n$, and $\ln n = \lg n \cdot \ln 2$, we get $C_n \approx 2 \ln 2 \cdot \lg n \approx 1.386 \lg n$.
∎
...
pairs the expected search cost drifts *up*, toward $\Theta(\sqrt{n})$ average path length,
not the $\Theta(\lg n)$ you started with. The fix in practice is to alternate
...
> node's **balance factor** is $B(P) = \operatorname{height}(\text{right}) - \operatorname{height}(\text{left}) \in \{-1, 0, +1\}$.

### Why balance implies $\sim 1.44 \lg n$ height — the Fibonacci tree
...
Since $F_k \approx \varphi^k / \sqrt{5}$ with $\varphi = (1+\sqrt{5})/2$, we invert $n \ge A_h = F_{h+3} - 1$:

$$h + 3 \le \log_\varphi(\sqrt{5}\,(n + 1)), \quad \text{so} \quad h \le 1.4404 \lg(n+1) + \text{const}.$$
...
An insertion can push one balance factor to $\pm 2$. We repair it at `S`, the
...
Comparison-based search is stuck at $\Omega(\lg n)$. **Hashing** sidesteps the bound:
...
$$h(K) = K \bmod M, \qquad M \text{ a prime}.$$
...
slot. The **load factor** $\alpha = N/M$ (fraction full) governs everything.
...
(We keep one slot always empty, $N \le M - 1$, so an unsuccessful search is
...
> **Successful search:**   $C \approx \frac{1}{2}\left(1 + \frac{1}{1 - \alpha}\right)$.
> **Unsuccessful search:** $C' \approx \frac{1}{2}\left(1 + \frac{1}{(1 - \alpha)^2}\right)$.

At $\alpha = 0.5$: successful $\approx 1.5$ probes, unsuccessful $\approx 2.5$. At $\alpha = 0.9$:
successful $\approx 5.5$, unsuccessful $\approx 50.5$(!). The unsuccessful cost blows up
quadratically as $\alpha \to 1$ — the signature of clustering. This is the analysis the
lab measures empirically: fill a 1009-slot table to $\alpha = 0.5$ and confirm the
average successful search is under 2 probes; fill to $\alpha = 0.9$ and watch it climb
past 5.
...
$$h_1(K) = K \bmod M, \quad c = h_2(K) = 1 + (K \bmod (M - 2)), \quad \text{probe } i \gets i - c \pmod M.$$

Since $M$ is prime and $1 \le c \le M - 2$, we have $\gcd(c, M) = 1$, so the probe
sequence visits **all** $M$ slots before repeating. Two keys with the same
$h_1$ almost always have different steps $c$, so they don't march in lockstep:
...
At $\alpha = 0.9$: successful $\approx 2.56$ probes vs. linear probing's $5.5$, and
unsuccessful $\approx 10$ vs. $50$. The lab inserts the *same* key set into both
tables at $\alpha = 0.9$ and confirms double hashing wins decisively.
...
cases, duplicate membership, and Theorem B's $\lfloor \lg N \rfloor + 1$ bound across many
...
sequences, ascending `1..=1000` and descending with `height` $\le 14$, 10 000
random keys `height` $\le 18$, duplicate rejection, and a `BTreeSet` model.

### Stage 4 — `LinearProbe`, `DoubleHash` (Algorithms 6.4L, 6.4D)

Division hashing $h(K) = K \bmod M$, decreasing probe sequences, full at
...
against a `HashSet` at $\alpha = 0.5$ and $0.9$, and check the *analysis*: linear
probing averages under 2 probes at $\alpha = 0.5$, climbs past 2.5 at $\alpha = 0.9$, and
double hashing beats linear probing on the same key set at $\alpha = 0.9$.
...
2. A BST built from already-sorted input has what height, and what does search
   cost then? ($n - 1$; $\Theta(n)$ — no better than sequential.)
...
4. An AVL tree has 100 nodes. Can its height be 10? 20? (No to both: the max is
   $1.4405 \lg 102 - 0.328 \approx 9.3$, so height $\le 9$; 10 and 20 are impossible.)
5. Why must a hash table's modulus $M$ be prime (and not near a power of 2)?
6. At $\alpha = 0.9$, why is linear probing's *unsuccessful* search ($\sim 50$ probes) so
   much worse than double hashing's ($\sim 10$)? (Primary clustering: runs grow
   quadratically in $1/(1-\alpha)$.)
...
| ▶6.2.1-3 | 22 | Prove Theorem B: at most $\lfloor \lg N \rfloor + 1$ comparisons, successful or not. |
...
| 6.4-8 | 22 | Analyse linear probing: derive $\frac{1}{2}(1 + 1/(1-\alpha))$ for successful search. |
...
are read directly off the $(1 + 1/(1-\alpha))$-shaped curves you measured in
...
- **Comparison-tree lower bounds** — $\lfloor \lg n \rfloor + 1$ is forced by counting leaves;
  the same tree gives the BST average-depth recurrence.
- **Extremal structures** — Fibonacci trees are precisely the skinniest
  legal AVL trees; height $\le 1.44 \lg n$ falls out of $F_h$ growth.
...
- **Comparison lower bounds** ($\lg N$) tie back to Module 06's sorting bound —
  same decision-tree argument, same $\Omega(n \lg n)$ / $\Omega(\lg n)$ walls.
...
- **Hashing** and its analysis (the $1/(1-\alpha)$ and $\ln(1/(1-\alpha))$ laws) reappear
