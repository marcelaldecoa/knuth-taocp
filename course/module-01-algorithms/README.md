...
5. **Effectiveness.** Each operation must be basic enough that a person with
   ...
   effective for integers; "print the exact decimal expansion of π" is not.
...
*Proof.* Write m = qn + r with r = m mod n. Any d dividing both m and n
divides r = m − qn; any d dividing both n and r divides m = qn + r. So the
pairs (m, n) and (n, r) have exactly the same set of common divisors — in
particular the same greatest one. ∎

**Termination.** After each execution of E3 the second operand strictly
decreases: n > r ≥ 0. A strictly decreasing sequence of non-negative integers
is finite, so E2 must eventually see r = 0. And when it does, gcd(m, n) =
gcd(n, 0) = n. ∎
...
(Q, I, Ω, f) where Q is a set of *states*, I ⊆ Q the inputs, Ω ⊆ Q the
outputs, and f : Q → Q a transition function that fixes every output state
(f(q) = q for q ∈ Ω). A computation is the sequence x, f(x), f(f(x)), …; it
*terminates* when it first hits Ω. An **algorithm** is a computational method
that terminates for every x ∈ I, with each f effective. Euclid's algorithm in
this dress: states are tuples (m, n, r, step-number); try writing f out once —
it is a five-minute exercise that makes the definition stick.
...
Initially (a1, b1) = (1, 0), (a, b) = (0, 1), (c, d) = (m, n). Each division
step c = qd + r replaces (c, d) by (d, r); restore the invariant by replacing
(a1, a) with (a, a1 − q·a) and likewise for b. When r = 0, return (d, a, b).

*Why this matters:* when gcd(m, n) = 1, the identity reads a·m ≡ 1 (mod n) —
extended Euclid *is* modular inversion, the workhorse of §4.5.2 and of all of
modern public-key cryptography. The stage tests use it exactly that way.

*Proof to write in your notebook:* the invariant holds initially and is
preserved by the update; at termination d = gcd by the Stage-1 lemma. That is
mathematical induction doing the work — §1.2.1's theme.

### Stage 4 — `division_steps` and Lamé's theorem

Let T(m, n) be the number of divisions (executions of E1) that Algorithm E
performs. How large can T be? This question, answered by Lamé in 1845, is
...
one before" is precisely the Fibonacci recurrence F₁ = F₂ = 1,
F_{k+1} = F_k + F_{k−1}.
...
The last pair before termination is (x, y) with y ≥ 1 and x ≥ 2y... in fact,
working backwards from the final division, the smallest inputs that can force
t divisions satisfy the Fibonacci recurrence, so n ≥ F_{t+1}. Contrapositive:
n < F_{k+1} implies t ≤ k − 1. Knuth gives the full argument (and much more —
the *average*, via continued fractions) in §4.5.3.

**Your job:** implement `division_steps`, then let the tests run the
experiment: exhaustively over all m, n < F₁₆ = 987, the worst case is 14
divisions and the first pair to achieve it is (377, 610) = (F₁₄, F₁₅) — the
swap costs one extra division, then the pure Fibonacci descent does 13.

The last test peeks at the average case: Knuth shows T(m, n) averages about
(12 ln 2 / π²) ln n ≈ 0.843 ln n. You verify only the order of magnitude —
...
1. Which of the five properties fails for "S1. Set x to a real number solving
   x² = 2"? (Effectiveness — and also definiteness: *which* solution?)
2. Why does Algorithm E terminate even when m < n initially?
3. If T(m, n) = 1, what can you say about m and n? (n divides m.)
4. Where exactly does the correctness proof use m = qn + r with 0 ≤ r < n —
   and which Rust operator guarantees it for `u64`?
