...
A boolean function f(x₀, …, x_{n−1}) and a family of subsets of
{0, …, n−1} are interchangeable in principle: the family is
F = { s : f(χ_s) = 1 } where χ_s is the bit vector of s. Module 13's BDDs
...
of the 2ⁿ subsets are *not* members, and each member set contains only a few
of the n elements. A BDD is forced to spend nodes saying "…and x₁₇ is 0, and
x₁₈ is 0, and x₁₉ is 0…" for every irrelevant variable, because deleting a
...
A **ZDD** over ordered variables x₀ < x₁ < … < x_{n−1} is a DAG with two
...
Keep ⊥ and ⊤ straight — they are as different as 0 and 1:

| | ∅ = F(⊥) | {∅} = F(⊤) |
|---|---|---|
| number of member sets | 0 | 1 |
| is ∅ a member? | no | yes |
| role in the algebra | identity of ∪, annihilator of ⊔ | identity of ⊔ |

### The two reduction rules

1. **Zero-suppression.** Never create a node with HI = ⊥. Such a node would
...
The family F = {{0,1}, {2}} over universe {0, 1, 2}. Split on x₀: members
...
subfamily doesn't contain. Read membership by walking: is {2} ∈ F? At
x₀: 0 ∉ {2}, go LO to the x₂ node; 2 ∈ {2}, go HI to ⊤; every wanted element
consumed: yes. Is {0} ∈ F? At x₀: go HI to the x₁ node — but now the wanted
set is exhausted while the node demands a decision about 1: go LO, reach ⊥:
no. And crucially, is {1} ∈ F? At x₀ we go LO to the x₂ node, which has
*skipped past* x₁ — by zero-suppression, no member set on this side contains
...
F of subsets of {0, …, n−1} is denoted by exactly one reduced,
zero-suppressed diagram.

*Proof sketch (induction on the number of available variables).* Zero
variables: the only families are ∅ and {∅}, denoted by the two sinks, which
are distinct. Now suppose the theorem holds for families over {v+1, …} and
let F be a family over {v, v+1, …}. Split F uniquely as

    F₀ = { s ∈ F : v ∉ s },      F₁ = { s \ {v} : s ∈ F, v ∈ s }.

Both are families over {v+1, …}, so each has a unique diagram by induction.
If F₁ = ∅ — v absent from every member — then F = F₀ and the unique diagram
of F is F₀'s: this is *exactly* the case zero-suppression forbids us to
build a node for, so the rule and the uniqueness argument are the same fact.
If F₁ ≠ ∅, the node (v, D(F₀), D(F₁)) denotes F, and any diagram for F
rooted at a v-node must have children denoting F₀ and F₁ (the semantics
forces it), unique by induction; the unique table makes the node itself
unique. Distinct families split into distinct (F₀, F₁) pairs, so distinct
...
Here is the trap Knuth warns about. In a BDD, f and ¬f have identical size —
...
F = {{i} : 0 ≤ i < 64} has a 66-node ZDD (one chain node per variable plus
the sinks — stage 1 pins this). Its *complement family* — all 2⁶⁴ − 64 other
...
### Counting members: no 2^skip factor

Module 13's Algorithm 7.1.4C had to multiply by 2^(skipped levels), because
...
⊤ (at each node, HI ⇔ "the branch variable is in the set").

*Proof.* Given a member set s, the walk used by `contains_set` traces a
unique path to ⊤, by induction on the semantics. Conversely a path to ⊤
...
Hence `count_sets` is the naked recursion c(⊥) = 0, c(⊤) = 1,
c(node) = c(LO) + c(HI) — path counting — memoized to O(Z(f)) additions.
...
the split of f at its top variable v as f = f₀ ∪ ({{v}} ⊔ f₁), i.e.
f₀ = F(LO), f₁ = F(HI).
...
|---|---|---|---|
| f ∪ g | mk(v, f₀ ∪ g, f₁) | mk(v, f ∪ g₀, g₁) | mk(v, f₀ ∪ g₀, f₁ ∪ g₁) |
| f ∩ g | f₀ ∩ g — f's HI dies wholesale | f ∩ g₀ | mk(v, f₀ ∩ g₀, f₁ ∩ g₁) |
| f \ g | mk(v, f₀ \ g, f₁) — g can't cancel v-sets | f \ g₀ — g's HI is idle | mk(v, f₀ \ g₀, f₁ \ g₁) |

with trivial cases ∅ ∪ g = g, ∅ ∩ g = ∅, f \ f = ∅, f ∪ f = f ∩ f = f, and
so on. Each `mk` may zero-suppress its result — that is the algebra
maintaining canonicity for free.

**Join** is the interesting one — Knuth's f ⊔ g (Minato's product):

    f ⊔ g  =  { a ∪ b : a ∈ f, b ∈ g }.

It is the Minkowski-sum idea transplanted to set systems: every way of
combining one choice from f with one from g. {∅} is its identity (join with
"choose nothing" changes nothing) and ∅ annihilates (no way to choose from
an empty menu). Derive the recursion by distributing joins over the top-
variable splits — join distributes over ∪ directly from its definition:

    f ⊔ g = (f₀ ∪ v·f₁) ⊔ (g₀ ∪ v·g₁)        writing v·h for {{v}} ⊔ h
          = f₀⊔g₀  ∪  v·(f₁⊔g₀)  ∪  v·(f₀⊔g₁)  ∪  v·v·(f₁⊔g₁)

and v·v = v ({v} ∪ {v} = {v}: elements don't double), so

    LO = f₀ ⊔ g₀,    HI = (f₁ ⊔ g₁) ∪ (f₁ ⊔ g₀) ∪ (f₀ ⊔ g₁).

When only f tests v, the split of g is (g, ∅) and this collapses to
mk(v, f₀ ⊔ g, f₁ ⊔ g). Note that join *calls union* — the memo table is
shared across operations, keyed by an op tag.

Two sanity anchors you will pin in stage 2: {{a},{b}} ⊔ {{c}} =
{{a,c},{b,c}}, and when f and g have disjoint supports, |f ⊔ g| = |f|·|g|
(distinct pairs give distinct unions — restrict a union to f's support to
recover a). The algebra (∪ as addition, ⊔ as multiplication) is a
commutative semiring with additive identity ∅ and multiplicative identity
{∅}; stage 2 verifies the semiring laws *as Ref equalities*, plus the
difference identities f \ (g ∪ h) = (f \ g) ∩ (f \ h) and
f \ (g ∩ h) = (f \ g) ∪ (f \ h) — De Morgan with ∁ replaced by \, which is
...
variables be the *edge indices* 0, …, m−1, and build the family of all
...
### Hand trace: matchings of P₃

P₃ is 0—1—2 with edges e₀ = {0,1}, e₁ = {1,2}; the only conflict pair is
(e₀, e₁) (they share vertex 1).
...
Three matchings of P₃ — and 3 = F₄. In general, condition on the last edge
of P_n: unused ⇒ a matching of P_{n−1}; used ⇒ its neighbor edge is banned,
leaving a matching of P_{n−2}. So m(P_n) = m(P_{n−1}) + m(P_{n−2}) with
m(P₁) = 1, m(P₂) = 2: the Fibonacci numbers, m(P_n) = F_{n+1}. For cycles,
condition on one fixed edge to break the ring: m(C_n) = m(P_{n−1}) +
m(P_{n−3})·1 + … — the tidy result is m(C_n) = L_n, the Lucas numbers, which
...
n ≤ 7, with the lovely recurrence T(n) = T(n−1) + (n−1)·T(n−2) — either the
...
in one pass; that is how people count 10⁶⁰ paths through grid graphs. Same
...
is XCC verbatim (stage 4's word-pair test is a 2×2 miniature). Without
...
items beyond a boundary N₁ in his item array — same idea, different
...
A = {p₀; s:R}, B = {p₁; s:R}: C3 picks p₀; C4 covers it; C5 takes A and
commits its s-node → purify: B's s-node has color R → marked −1. Recurse:
C3 picks p₁, whose list still contains B; committing B's s-node is a no-op
(−1). Root ring empty: solution {A, B}. Now replay with B' = {p₁; s:G}:
purify sees G ≠ R → hide(B'), which unlinks B' from *p₁'s* list; at the
next level p₁ has an empty list — dead end, zero solutions. Same-color
compatible, different-color not: the semantics fell out of the pointer
dance.

### Latin squares, the canonical XCC citizen

An n×n **Latin square** fills each cell with one of n symbols so every row
and every column contains each symbol exactly once. As exact cover: items
cell(r,c), row-sym(r,s), col-sym(c,s) — all primary, 3n² of them; the
...
visits solutions one at a time, the ZDD-building variant can return 10¹⁵ of
...
- **Graphillion** (Inoue–Iwashita–Kawahara–Minato) is a production Python
  ...
  (and the viral "counting paths on a grid" video that made 10⁶⁰ a pop-science
...
∅ vs {∅}). `count_sets` is the LO+HI recursion; `contains_set` is one walk
...
against `HashSet<BTreeSet<u32>>` brute force for n ≤ 8; if a law fails but
...
conflicts with `diff` of a `join` (G2), count (G3) — trace P₃ by hand
...
Fibonacci, Lucas, telephone numbers, brute-force tiny graphs, and the 4×4
...
1. What family does the one-node diagram `(x3, ⊤, ⊤)` denote, and why
   would this node be illegal in Module 13's BDD? ({∅, {3}}; a BDD elides
   any node with lo = hi.)
2. `mk(5, f, empty())` returns… what, and which theorem needs that
   behavior? (Returns f; uniqueness of the canonical form — the F₁ = ∅
   case of the induction.)
3. Why does `count_sets` have no 2^skip factor where Module 13's
   `count_models` did? (ZDD paths to ⊤ biject with member sets; a skipped
...
|---|---|---|
| 7.1.4-(z-vs-b) | 18 | Draw both the BDD and the ZDD of the family {{1},{2},{3}} over universe {1,…,6}. Which nodes does each elide, and why? |
...
| ▶7.1.4-(join-alg) | 25 | Prove that (families, ∪, ⊔) is a commutative semiring. Then investigate size: show \|f ⊔ g\| = \|f\|·\|g\| for disjoint supports, and construct f, g over a shared support whose join has far more members than Z(f) + Z(g) nodes would suggest. |
...
- **Canonicity by structural induction** — the ZDD canonical-form theorem
  ...
  (F₁ = ∅ ↔ the reduction rule) is the technique: *a canonical form is a
  bijection theorem, and each reduction rule discharges exactly one
  non-uniqueness*.
- **Bijective counting** — members ↔ ⊤-paths turned `count_sets` into path
...
- **Reversibility invariants** — XCC correctness rests on "undo sees what
  do saw": a one-bit state machine per node (c ↔ −1) nested inside a LIFO
...
- **Frontier-based construction** (Simpath and Graphillion's engine) is
  the scalable path from stage 3's algebra to 10⁶⁰-object families.
