...
$$-x = \bar{x} + 1 \qquad \text{(because } x + \bar{x} = 111\ldots1 = 2^{64} - 1 \equiv -1\text{)}.$$

**Lemma (rightmost-bit extraction).** If $x \ne 0$, write $x$ in binary as

$$x = \alpha\, 1\, 0^a$$

where $a$ = number of trailing zeros and $\alpha$ is the remaining $63 - a$ high bits.
Then $x \mathbin{\&} (-x) = 2^a$: the AND isolates exactly the rightmost 1-bit.

*Proof.* Complement each region: $\bar{x} = \bar{\alpha}\, 0\, 1^a$. Add 1: the carry ripples
through the $a$ trailing ones and stops at the 0, giving

$$-x = \bar{x} + 1 = \bar{\alpha}\, 1\, 0^a.$$

Now AND with $x = \alpha\, 1\, 0^a$: the high region gives $\alpha \mathbin{\&} \bar{\alpha} = 0$, the middle bit
gives $1 \mathbin{\&} 1 = 1$, the low region gives 0. Result: $0\ldots010^a = 2^a$. ∎

The same three-line computation proves a whole family. With $x = \alpha\, 1\, 0^a$ we
have $x - 1 = \alpha\, 0\, 1^a$, hence:

| expression   | result (binary)         | effect                                   |
|--------------|-------------------------|------------------------------------------|
| `x & (x-1)`  | $\alpha\, 0\, 0^a$      | *drop* the rightmost 1                    |
| `x \| (x-1)` | $\alpha\, 1\, 1^a$      | *smear* the rightmost 1 to the right      |
| `x ^ (x-1)`  | $0\, 1\, 1^a$           | mask of the rightmost 1 and all below     |
| `x & (-x)`   | $0\, 1\, 0^a$           | *extract* the rightmost 1                 |

Hand-check every row on $x = 01011000$ (so $\alpha = 0101$, $a = 3$) before you code
stage 1; the tests use exactly this example.

Two boundary conventions, fixed by the wrapping arithmetic: for $x = 0$ the
extraction gives $0 \mathbin{\&} 0 = 0$ (nothing to extract), and the smear gives
$0 \mathbin{|} (2^{64} - 1)$ = all ones. Rust makes you say `wrapping_neg`/`wrapping_sub`
...
$\nu(x)$, the number of 1-bits of $x$ ("population count", "sideways addition"
...
is cheaper. For a 2-bit field with value $v \in \{0, 1, 2, 3\}$:

| $v$ (binary) | $\lfloor v/2 \rfloor$ | $v - \lfloor v/2 \rfloor$ | $\nu(v)$ |
|---|---|---|---|
| 00 | 0 | 0 | 0 |
| 01 | 0 | 1 | 1 |
| 10 | 1 | 1 | 1 |
...
So $\nu(v) = v - \lfloor v/2 \rfloor$ on each field, and `(x >> 1) & 0x5555…` computes all
...
Trace one byte of it, $x = 10110110$ ($\nu = 5$):
...
$\rho(x)$ = the number of trailing zeros of $x$ — the exponent $a$ of the lemma in
§1.1. The sequence $\rho(1), \rho(2), \rho(3), \ldots = 0, 1, 0, 2, 0, 1, 0, 3, \ldots$ rises
like the tick marks on a ruler, whence the name. $\rho(0)$ is *undefined* — there
is no rightmost 1-bit — and your implementation must say so loudly
(definiteness, as always since Module 01).

You already know how to reduce $\rho$ to a simpler problem: $b = x \mathbin{\&} (-x) = 2^a$,
...
`x & 0xFFFFFFFF` $\ne 0$ the rightmost 1 is in the low 32 bits, else add 32 to the
...
Combinatorics wants to enumerate all $\binom{n}{w}$ words with exactly $w$ bits set —
...
$$x = \alpha\, 0\, 1^b\, 0^a \qquad (b \ge 1 \text{ ones, then } a \ge 0 \text{ zeros}).$$
...
**Algorithm G** (*Gosper's hack*). Given $x \ne 0$, return the next larger word
of the same weight.
...
Trace $x = 01011100$ ($\alpha = 010$, $b = 3$, $a = 2$; weight 4):
...
`/ u` can be replaced by a shift by $\rho(u)$ — but the division form is the
...
behavior for the other. When you enumerate $\binom{12}{3}$ subsets in the tests,
...
Fix a variable order $x_0 < x_1 < \ldots < x_{n-1}$. A **binary decision diagram**
is a rooted DAG with two *sinks* $\bot$ and $\top$ and internal *branch nodes*; each
...
root and follow branches; you reach a sink in $\le n$ steps. $B(f)$ denotes the
number of nodes (sinks included) reachable from $f$'s root.

Here is the median (2-out-of-3 majority) $\langle x_0\, x_1\, x_2 \rangle$, LO edges drawn left:
...
The two $x_2$-nodes drawn are the *same* node (both are $(x_2, \bot, \top)$); rule R2
merges them, giving $B = 6$: one $x_0$-node, two $x_1$-nodes, one $x_2$-node, 2 sinks.

### 2.2 The canonicity theorem — the module's central theorem

**Theorem (Bryant 1986; §7.1.4).** Fix the variable order. Every Boolean
function $f$ of $x_0, \ldots, x_{n-1}$ is represented by *exactly one* reduced
ordered BDD.
...
reduced diagrams with no branch nodes are the bare sinks $\bot$ and $\top$. Unique,
and distinct from each other.

*Step.* Assume the theorem for functions of $x_1, \ldots, x_{n-1}$ (one fewer
variable). Let $f$ be a function of $x_0, \ldots, x_{n-1}$ and let $f_0 = f|_{x_0=0}$ and
$f_1 = f|_{x_0=1}$ be its cofactors — both functions of $x_1, \ldots, x_{n-1}$ only, so
each has a unique ROBDD by the induction hypothesis.

*Case 1: $f_0 = f_1$.* Then $f$ does not depend on $x_0$. No ROBDD for $f$ can have a
root labeled $x_0$: such a root's LO and HI subgraphs would be ROBDDs for $f_0$
and $f_1$ (the subgraph rooted at any node is itself reduced and ordered, and
...
*Case 2: $f_0 \ne f_1$.* Then $f$ depends on $x_0$, so the root must be labeled $x_0$ —
a root labeled $x_j$ with $j \ge 1$ would make the whole diagram a function
independent of $x_0$ (ordering: $x_0$ can never appear below $x_j$). The root's LO
and HI subgraphs are ROBDDs of $f_0$ and $f_1$, unique by induction, and they are
non-identical since $f_0 \ne f_1$ — so R1 is satisfied, and by R2 there is exactly
one node with this $(x_0, \mathrm{LO}, \mathrm{HI})$ triple. The whole diagram is determined. ∎
...
$f \equiv g \iff$ `Ref(f) == Ref(g)` — equivalence testing in $O(1)$.

Satisfiability is $f \ne \bot$; tautology is $f = \top$. Stage 2's tests take the
...
operations, using the **Shannon expansion**. For any binary operation $\diamond$ and
the topmost (smallest) variable $v$ appearing in $f$ or $g$:

$$f \diamond g = \left( \bar{x}_v \land (f_0 \diamond g_0) \right) \lor \left( x_v \land (f_1 \diamond g_1) \right),$$

where $f_0, f_1$ are $f$'s cofactors on $x_v$ — which are just `low(f)`, `high(f)`
if $f$'s root tests $v$, and $f$ itself otherwise ($f$ doesn't depend on $x_v$; its
cofactors are trivial). The operation $\diamond$ commutes with cofactoring, which is
all this identity says.

**Algorithm A** (*Apply, §7.1.4*). Given nodes $f, g$ in a shared arena and
$\diamond \in \{\land, \lor, \oplus\}$, compute the node of $f \diamond g$.
...
$O(B(f) \cdot B(g))$ node expansions.
...
own descendants or leaves it in place. So there are at most $B(f) \cdot B(g)$
...
operation — including $\lnot f$, which is simply $f \oplus \top$ (one memoized $O(B(f))$
sweep). De Morgan's laws hold as *pointer equalities* in your arena.

### 2.4 Counting models (Algorithm 7.1.4C) — and the $2^{\text{skip}}$ lemma
...
variables $x_{v+1}, \ldots, x_{w-1}$ without testing them.

Define the *level* $\ell(p)$ of a branch node as its variable index, and pin the
sinks at $\ell = n$. For each node $p$ let $s(p)$ = the number of assignments to the
variables $x_{\ell(p)}, \ldots, x_{n-1}$ that satisfy $p$'s function. (This is
well-defined: $p$'s function depends only on those variables.)

**Lemma (skip weighting).** $s(\bot) = 0$, $s(\top) = 1$, and for a branch node $p$ at
level $v$ with children $l = \mathrm{LO}(p)$, $h = \mathrm{HI}(p)$:

$$s(p) = 2^{\ell(l)-v-1} \cdot s(l) + 2^{\ell(h)-v-1} \cdot s(h).$$
...
satisfy $l$'s function, which depends only on $x_{\ell(l)}, \ldots, x_{n-1}$. The
variables $x_{v+1}, \ldots, x_{\ell(l)-1}$ — the *skipped* ones — are therefore
unconstrained: every assignment counted by $s(l)$ (well-defined by induction,
since $\ell(l) > v$) extends to exactly $2^{\ell(l)-v-1}$ assignments of
$x_{v+1}, \ldots, x_{n-1}$, each giving one assignment counted by $s(p)$ with
...
$$\#\mathrm{models}(f \text{ over } n \text{ vars}) = 2^{\ell(f)} \cdot s(f)$$
...
Hand-trace on the median BDD of §2.1. Name the nodes: $Z = (x_2, \bot, \top)$,
$A = (x_1, \bot, Z)$, $B = (x_1, Z, \top)$, root $R = (x_0, A, B)$; sinks live at level 3.
Note the two edges that *skip* level 2: $A$'s LO edge (level 1 → $\bot$ at level 3
jumps over $x_2$) and $B$'s HI edge (level 1 → $\top$, likewise).

| node | level | s, factor by factor |
|---|---|---|
| $\bot$ | 3 | 0 |
| $\top$ | 3 | 1 |
| $Z$ | 2 | $2^{3-2-1} \cdot s(\bot) + 2^{3-2-1} \cdot s(\top) = 1 \cdot 0 + 1 \cdot 1 = 1$ |
| $A$ | 1 | $2^{3-1-1} \cdot s(\bot) + 2^{2-1-1} \cdot s(Z) = 2 \cdot 0 + 1 \cdot 1 = 1$ |
| $B$ | 1 | $2^{2-1-1} \cdot s(Z) + 2^{3-1-1} \cdot s(\top) = 1 \cdot 1 + 2 \cdot 1 = 3$ |
| $R$ | 0 | $2^{1-0-1} \cdot s(A) + 2^{1-0-1} \cdot s(B) = 1 + 3 = 4$ |

Answer: $2^{\ell(R)} \cdot s(R) = 2^0 \cdot 4 =$ **4** ✓ — the models are 011, 101, 110,
111. Sanity-check $B$'s row against the picture: from $B$, taking $x_1 = 1$ goes
straight to $\top$ *without testing $x_2$*, so both 110 and 111 are models — that
HI edge is worth 2, not 1. Forgetting exactly that factor (getting 3) is
the classic stage-3 bug; the reduction rule R1 deleted the node $(x_2, \top, \top)$,
...
$$f = (x_{a_1} \land x_{b_1}) \lor (x_{a_2} \land x_{b_2}) \lor \ldots \lor (x_{a_k} \land x_{b_k})$$

over $2k$ distinct variables.

**Good order** (couples adjacent): $a_1 b_1 a_2 b_2 \ldots a_k b_k$. Reading the
...
**Bad order** (all firsts, then all seconds): $a_1 a_2 \ldots a_k b_1 b_2 \ldots b_k$. Now,
after reading all the a's, the *entire subset* $S = \{ i : x_{a_i} = 1 \}$ is
...
*Proof.* For each of the $2^k$ assignments $\alpha$ to the a-variables, let $f_\alpha$ be
the residual function of the b-variables. Explicitly $f_\alpha = \bigvee_{i \in S(\alpha)} x_{b_i}$
where $S(\alpha) = \{ i : \alpha(a_i) = 1 \}$. These $2^k$ functions are *pairwise
distinct*: if $S(\alpha) \ne S(\alpha')$, pick $i$ in one but not the other; the b-assignment
"$x_{b_i} = 1$, all other b's $= 0$" satisfies exactly one of $f_\alpha, f_{\alpha'}$.

Now walk the BDD from the root following $\alpha$ (taking the branch $\alpha$ dictates at
...
at or below the a/b boundary; call it $p(\alpha)$. The function computed from $p(\alpha)$
by the remaining walk is exactly $f_\alpha$ — this is the defining semantics of a
decision diagram: the residual function is determined by the current node
alone, not by how you got there. Therefore $\alpha \mapsto p(\alpha)$ maps $2^k$ *distinct*
...
comes out at exactly 512 nodes while the good order gives exactly $2 \cdot 8 + 2 = 18$,
with **the model count identical ($2^{16} - 3^8 = 58975$) in both**, since
...
nodes with LO = HI, delete nodes whose **HI edge points to $\bot$** — i.e. a
...
misprediction, and it vectorizes: the same code counts $2 \times 64$ bits in a 128-bit
...
Because the algebra says so: $x \mathbin{\&} (-x)$ is a total function with a natural
...
non-redundant. The $2^{\text{skip}}$ factors are the price of the compression R1
...
- *Hardware grew these functions because software proved them hot.* x86's
  ...
  $x \mathbin{\&} (-x)$, `BLSR` is $x \mathbin{\&} (x - 1)$, `BLSMSK` is $x \oplus (x - 1)$, `TZCNT` is
  ...
  matching bits: $\nu(a \oplus b)$ is Hamming distance), and today it powers bitmap
...
Algorithm C with the $2^{\text{skip}}$ weighting, `u128` arithmetic, memoized per node
...
$\bigwedge \lnot(x_u \land x_v)$ over edges; queens adds at-least-one-per-row disjunctions
over $n^2$ cell variables. The tests anchor the counts to closed forms —
...
2. The de Bruijn constant for `ruler` must begin with six 0 bits. Which
   ...
   $D \ll a$ pull "wrapped" bits into the top window — and what fills the
...
3. In the canonicity proof, point to the exact sentence where rule R1 is
   ...
   as simple as $x_3$ and $x_5$.)*
4. The BDD of $f = x_2$ over $n = 6$ variables is one branch node and two sinks.
...
|---|---|---|
| 7.1.3-(warmup) | 10 | Prove the four rightmost-bit identities of §1.1's table from $x = \alpha 1 0^a$, including what each does when $x = 0$. |
| ▶7.1.3-20 | 20 | Derive Gosper's hack yourself from the $\alpha 0 1^b 0^a$ picture, then prove the division in G3 is always exact. |
| 7.1.3-(swar) | 22 | Adapt Algorithm S to count bits in each *byte* separately (stop after S3) and use it to compute $\nu$ for a 512-bit block with one final multiply per word. |
...
| 7.1.4-(median) | 15 | Draw the ROBDD of $\langle x_1 x_2 x_3 \rangle$ under both orders $x_1 < x_2 < x_3$ and $x_3 < x_1 < x_2$; verify $B = 6$ both times (symmetric functions are order-insensitive — prove it). |
| ▶7.1.4-(canon) | 25 | Write out the canonicity induction for $n = 2$ explicitly: list all 16 functions and their unique diagrams. |
| 7.1.4-(count) | 20 | Prove the ZDD variant of the skip lemma (skipped level $\Rightarrow$ factor 1 on the LO-interpretation) and adapt Algorithm C. |
| ▶7.1.4-(hidden) | 30 | The *hidden weighted bit* function $\mathrm{HWB}(x) = x_{\nu(x)}$ (0 if $x = 0$): show every variable order gives exponential BDDs (Bryant 1991) — read the proof and reproduce its distinguishing-set core in your own words. |

## 8. Proof techniques you practiced

- **Representation-shape arguments** (write $x = \alpha 1 0^a$, compute both sides):
...
- **Exhaustive case analysis on a small domain:** the 4-row table proving
  $\nu(v) = v - \lfloor v/2 \rfloor$; small enough to be a *complete* proof, not an example.
...
- **Downward induction on levels with an accounting identity:** the $2^{\text{skip}}$
